// Exact XenosRecomp translation of GTA IV's PS_BinkNoAlpha container
// (cache hash 0xA6C9E2B8B2A59D7A). Kept as an additive native-cache entry.
...
)
{
	...
	r0.w = (float)((dot(r1.yxz, YUVtoRGB(2).zxy)));
	ps = YUVtoRGB(3).x * ps;
	r0.y = ps;
	r1.xyzw = (float4)((selectWrapper(c255.xxxy == 0.0, r1.yxzz, YUVtoRGB(3).xxxx)));
	output.oC0.y = (float)((dot(r1.xyzw, YUVtoRGB(1).zxyw)));
	output.oC0.xz = (float2)((r0.zw + r0.xy));
	ps = max(YUVtoRGB(3).w, YUVtoRGB(3).w);
	output.oC0.w = ps;
