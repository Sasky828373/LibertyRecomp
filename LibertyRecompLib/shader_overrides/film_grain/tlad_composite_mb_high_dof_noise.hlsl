// Exact XenosRecomp translation of TLAD PS_GTACompositeMBHighDOFNoise. The
...
float LibertySampleTladFilmGrain(
#ifdef __air__
	constant Texture2DDescriptorHeap* texture_heap,
	constant SamplerDescriptorHeap* sampler_heap,
#endif
	float2 pixel_position, float2 temporal_offset)
{
#ifdef __air__
	uint2 texture_size = getTexture2DDimensions(
		texture_heap[BlurSampler_Texture2DDescriptorIndex].tex);
#else
	uint2 texture_size = getTexture2DDimensions(
		g_Texture2DDescriptorHeap[BlurSampler_Texture2DDescriptorIndex]);
#endif
	float2 dimensions = max(float2(texture_size), float2(1.0, 1.0));
	const float grain_texel_size = 1.5625;
	float2 temporal_shift = floor(frac(temporal_offset) * dimensions);
	float2 grain_position = (pixel_position - float2(0.5, 0.5)) / grain_texel_size;
	float2 grain_uv = frac((grain_position + temporal_shift + 0.5) / dimensions);
	return tfetch2D(
#ifdef __air__
		texture_heap, sampler_heap,
#endif
		BlurSampler_Texture2DDescriptorIndex, BlurSampler_SamplerDescriptorIndex,
		grain_uv, float2(0, 0), GetTextureLodBias(3)).z;
}
...
)
{
...
#endif
#ifdef __air__
	float4 c249 = as_type<float4>(uint4(0x3E800000, 0x3F8CCCCD, 0x3F600000, 0xBF800000));
#else
	float4 c249 = asfloat(uint4(0x3E800000, 0x3F8CCCCD, 0x3F600000, 0xBF800000));
#endif
#ifdef __air__
	float4 c250 = as_type<float4>(uint4(0x43800000, 0x3F000000, 0x3FE66666, 0x404CCCCD));
#else
	float4 c250 = asfloat(uint4(0x43800000, 0x3F000000, 0x3FE66666, 0x404CCCCD));
#endif
#ifdef __air__
	float4 c251 = as_type<float4>(uint4(0x0, 0x3F800000, 0x3EC00000, 0x3F000000));
#else
	float4 c251 = asfloat(uint4(0x0, 0x3F800000, 0x3EC00000, 0x3F000000));
#endif
#ifdef __air__
	float4 c252 = as_type<float4>(uint4(0x3F200000, 0x3F400000, 0x423C851F, 0x4268A7F0));
#else
	float4 c252 = asfloat(uint4(0x3F200000, 0x3F400000, 0x423C851F, 0x4268A7F0));
#endif
#ifdef __air__
...
#endif
		StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r0.xy, float2(0, 0), GetTextureLodBias(7)).z;
	r10.z = tfetch2D(
#ifdef __air__
		g_Texture2DDescriptorHeap,
		g_SamplerDescriptorHeap,
	...
	r1.yzw = tfetch2D(
		...
		g_SamplerDescriptorHeap,
#endif
		BloomSampler_Texture2DDescriptorIndex, BloomSampler_SamplerDescriptorIndex, r0.xy, float2(0, 0), GetTextureLodBias(4)).xyz;
	r10.xy = (float2)((r0.yx * c254.zz + c249.ww));
	r12.w = (float)((-r10.x * dofProj.w));
	r4.w = (float)((dot(r9.zxy, c253.xyz)));
	r3.x = (float)((dot(r8.zxy, c253.xyz)));
	r3.y = (float)((dot(r7.zxy, c253.xyz)));
	r3.z = (float)((dot(r2.wyz, c253.xyz)));
	r3.w = (float)((dot(r6.zxy, c253.xyz)));
	ps = -abs(r0.x) > 0.0;
	r1.x = ps;
	r11.xyzw = (float4)((r2.xyzw * c250.xyyy));
	ps = dofProj.y * dofProj.x;
	r5.w = ps;
	r14.xyzw = (float4)((selectWrapper(c251.xyyy == 0.0, r4.wwww, r3.xxwy)));
	r13.xyzw = (float4)((c254.xyzw >= r11.xxxx));
	ps = -r11.x >= 0.0;
	r0.z = ps;
	r2.w = (float)((r0.z + -r13.z));
	ps = dofBlur.x - dofBlur.y;
	r4.x = ps;
	r10.w = (float)((dot(r14.yzwx, c249.xxxx)));
	ps = r13.x - r13.w;
	r2.x = ps;
	r3.xyzw = (float4)((-r10.wwww + r3.xywz));
	ps = r13.z - r13.y;
	r2.y = ps;
	r12.xyz = (float3)((r10.yzz * dofProj.zyx));
	ps = r13.y - r13.x;
	r2.z = ps;
	r0.z = (float)((dot(r2.xzyw, c253.wwww)));
	ps = r12.y - r12.z;
	r4.z = ps;
	r2.xyzw = (float4)((r2.wyxz * c253.wwww));
	ps = c251.w + r0.z;
	r4.y = ps;
	r5.xyz = (float3)((r2.xxx * r6.zyx + r11.wzy));
	r5.xyz = (float3)((r2.yyy * r7.zyx + r5.xyz));
	r0.z = (float)((r4.z + dofProj.x));
	ps = clamp(rcp(r4.y), -FLT_MAX, FLT_MAX);
	r2.x = ps;
	r3.w = (float)((r3.w * r3.w));
	ps = clamp(rcp(r0.z), -FLT_MAX, FLT_MAX);
	r2.y = ps;
	r5.xyz = (float3)((r2.www * r8.zyx + r5.xyz));
	r5.xyz = (float3)((r2.zzz * r9.zyx + r5.xyz));
	r5.xyzw = (float4)((r5.xyzw * r2.xxxy));
	ps = dofBlur.z - dofBlur.y;
	r4.z = ps;
	r2.w = (float)((r0.w >= PLAYER_MASK.x));
	ps = max(r5.w, r5.w);
	r0.z = ps;
	r11.w = (float)((r5.w + -dofDist.w));
	ps = dofDist.w - r0.z;
	r11.y = ps;
	r11.xz = (float2)((r12.wx * r5.ww));
	ps = clamp(rcp(dofDist.x), -FLT_MAX, FLT_MAX);
	r0.z = ps;
	r2.xyz = (float3)((-r5.www * motionBlurMatrix(2).zxy + motionBlurMatrix(3).zxy));
	r2.xyz = (float3)((r11.xxx * motionBlurMatrix(1).zxy + r2.xyz));
	r11.xy = (float2)((-dofDist.yy * c251.ww + r11.yw));
	r2.xyz = (float3)((r11.zzz * motionBlurMatrix(0).zxy + r2.xyz));
	r4.y = (float)((dot(r3.xzy, r3.xzy)));
	ps = dofProj.w * r2.x;
	r3.y = ps;
	r3.x = (float)((-r2.x * dofProj.z));
	ps = clamp(rcp(dofDist.z), -FLT_MAX, FLT_MAX);
	r0.w = ps;
	r11.xy = (float2)((max(r11.xy, c251.xx)));
	ps = clamp(rcp(r3.x), -FLT_MAX, FLT_MAX);
	r3.x = ps;
	r0.zw = (float2)((r11.xy * r0.zw));
	ps = clamp(rcp(r3.y), -FLT_MAX, FLT_MAX);
	r3.y = ps;
	r0.zw = (float2)((r0.zw * r4.xz + dofBlur.yy));
	r4.xz = (float2)((r2.yz * r3.xy));
	ps = Exposure.x * r1.w;
	r3.z = ps;
	r0.zw = (float2)((min(r0.zw, dofBlur.xz)));
	ps = Exposure.x * r1.z;
	r3.y = ps;
	r10.yzw = (float3)((-r10.yxw + r4.xzw));
	ps = Exposure.x * r1.y;
	r3.x = ps;
	r1.yw = (float2)((r10.yz * gDirectionalMotionBlurLength.xx));
	ps = max(r0.z, r0.w);
	r10.x = ps;
	r0.zw = (float2)((r10.xw * r10.xw));
	ps = c255.x * r1.y;
	r4.x = ps;
	r1.z = (float)((r4.y + r0.w));
	ps = c255.x * r1.w;
	r4.y = ps;
	r1.z = (float)((r3.w >= r1.z));
	ps = c251.y - r0.z;
	r2.x = ps;
	r0.w = (float)((r2.x * r1.z + r0.z));
	r1.z = (float)((r0.w * c249.x));
	ps = c251.y - r0.w;
	r0.z = ps;
	r2.xyz = (float3)((r1.zzz * r9.zyx));
	p0 = r2.w == 0.0;
	ps = p0 ? 0.0 : 1.0;
	r2.xyz = (float3)((r1.zzz * r8.zyx + r2.xyz));
	r2.xyz = (float3)((r1.zzz * r7.zyx + r2.xyz));
	r2.xyz = (float3)((r1.zzz * r6.zyx + r2.xyz));
	r2.xyz = (float3)((r5.zyx * r0.zzz + r2.zyx));
	if (p0)
	{
	...
	if (p0)
	{
		r4.zw = (float2)((r0.xy * c252.wz + r4.zw));
	}
	if (p0)
	{
		r0.z = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			JitterSampler_Texture2DDescriptorIndex, JitterSampler_SamplerDescriptorIndex, r4.zw, float2(0, 0), GetTextureLodBias(6)).x;
	}
	if (p0)
	{
		ps = -c251.w - -r0.z;
	...
	if (p0)
	{
		r14.xyzw = (float4)((r1.ywyw * c251.zzww + r4.zwzw));
	}
	if (p0)
	{
	...
	if (p0)
	{
		r6.xyz = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			HDRSampler_Texture2DDescriptorIndex, HDRSampler_SamplerDescriptorIndex, r15.zw, float2(0, 0), GetTextureLodBias(2)).xyz;
	}
	if (p0)
	{
		r7.xyz = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			HDRSampler_Texture2DDescriptorIndex, HDRSampler_SamplerDescriptorIndex, r15.xy, float2(0, 0), GetTextureLodBias(2)).xyz;
	}
	if (p0)
	{
		r8.xyz = tfetch2D(
	...
	if (p0)
	{
		r5.yzw = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			HDRSampler_Texture2DDescriptorIndex, HDRSampler_SamplerDescriptorIndex, r9.zw, float2(0, 0), GetTextureLodBias(2)).xyz;
	}
	if (p0)
	{
		r4.yzw = tfetch2D(
	...
	if (p0)
	{
		r12.x = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r13.xy, float2(0, 0), GetTextureLodBias(7)).z;
	}
	if (p0)
	{
		r12.y = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r15.xy, float2(0, 0), GetTextureLodBias(7)).z;
	}
	if (p0)
	{
		r12.z = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r15.zw, float2(0, 0), GetTextureLodBias(7)).z;
	}
	if (p0)
	{
		r9.x = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r14.zw, float2(0, 0), GetTextureLodBias(7)).z;
	}
	if (p0)
	{
		r9.y = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r14.xy, float2(0, 0), GetTextureLodBias(7)).z;
	}
	if (p0)
	{
		r9.z = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r9.zw, float2(0, 0), GetTextureLodBias(7)).z;
	}
	if (p0)
	{
		r9.w = tfetch2D(
			...
			g_SamplerDescriptorHeap,
#endif
			StencilCopySampler_Texture2DDescriptorIndex, StencilCopySampler_SamplerDescriptorIndex, r13.zw, float2(0, 0), GetTextureLodBias(7)).z;
	}
	if (p0)
	{
	...
	if (p0)
	{
		r0.z = (float)((dot(r1.yz, r1.yz) + c251.x));
	}
	if (p0)
	{
	...
	if (p0)
	{
		r5.x = (float)((dot(-r1.zyw, c251.yyy)));
	}
	if (p0)
	{
		r4.x = (float)((dot(-r9.zwyx, c251.yyyy)));
	}
	if (p0)
	{
		r1.yzw = (float3)((-r1.zyw + c251.yyy));
	}
	if (p0)
	{
		r9.xyzw = (float4)((-r9.ywxz + c251.yyyy));
		ps = sqrt(abs(r0.z));
		r0.z = ps;
	}
	if (p0)
	...
	r0.xy = (float2)((r0.xy * c250.wz + NoiseParams.xy));
	...
	r0.x = tfetch2D(
		...
		g_SamplerDescriptorHeap,
#endif
		AdapLumSampler_Texture2DDescriptorIndex, AdapLumSampler_SamplerDescriptorIndex, r1.xx, float2(0, 0), GetTextureLodBias(5)).x;
	r0.y = LibertySampleTladFilmGrain(
#ifdef __air__
		g_Texture2DDescriptorHeap,
		g_SamplerDescriptorHeap,
#endif
		input.iPos.xy, NoiseParams.xy);
	r0.y = (float)((r0.y + -c251.w));
	...
	r0.xyz = (float3)((max(r0.xyz, c251.xxx)));
	r1.xyz = (float3)((r0.xyz * ToneMapParams.zzz));
	r1.xyzw = (float4)((r1.xyzw * c249.xxxy));
