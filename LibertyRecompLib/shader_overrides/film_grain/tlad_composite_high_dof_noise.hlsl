...
)
{
...
#endif
#ifdef __air__
	float4 c251 = as_type<float4>(uint4(0x43800000, 0x3F000000, 0x3FE66666, 0x404CCCCD));
#else
	float4 c251 = asfloat(uint4(0x43800000, 0x3F000000, 0x3FE66666, 0x404CCCCD));
#endif
#ifdef __air__
	float4 c252 = as_type<float4>(uint4(0x0, 0x3F800000, 0xBF000000, 0x3F000000));
#else
	float4 c252 = asfloat(uint4(0x0, 0x3F800000, 0xBF000000, 0x3F000000));
#endif
#ifdef __air__
	...
	r0.zw = (float2)((r0.xy * c251.wz + NoiseParams.xy));
	r1.xy = (float2)((frac(r0.zw)));
	ps = -abs(r0.x) > 0.0;
	r0.z = ps;
	r8.y = LibertySampleTladFilmGrain(
#ifdef __air__
	...
	r0.z = tfetch2D(
		...
		g_SamplerDescriptorHeap,
#endif
		AdapLumSampler_Texture2DDescriptorIndex, AdapLumSampler_SamplerDescriptorIndex, r0.zz, float2(0, 0), GetTextureLodBias(5)).x;
	r0.w = tfetch2D(
#ifdef __air__
		g_Texture2DDescriptorHeap,
		g_SamplerDescriptorHeap,
...
#endif
		HDRSampler_Texture2DDescriptorIndex, HDRSampler_SamplerDescriptorIndex, r0.xy, float2(-1.5, 0.5), GetTextureLodBias(2)).xyz;
	r4.w = (float)((dot(r7.zxy, c253.xyz)));
	r1.x = (float)((dot(r6.zxy, c253.xyz)));
	r1.y = (float)((dot(r5.zxy, c253.xyz)));
	r1.z = (float)((dot(r9.wyz, c253.xyz)));
	r1.w = (float)((dot(r4.zxy, c253.xyz)));
	r3.xy = (float2)((r0.ww * dofProj.yx));
	ps = clamp(rcp(r0.z), -FLT_MAX, FLT_MAX);
	r0.x = ps;
	r10.xyzw = (float4)((r9.xyzw * c251.xyyy));
	ps = dofProj.y * dofProj.x;
	r3.w = ps;
	r12.xyzw = (float4)((selectWrapper(c252.xyyy == 0.0, r4.wwww, r1.xxwy)));
	r0.z = (float)((-r10.x >= c252.x));
	ps = ToneMapParams.y * r0.x;
	r0.w = ps;
	r11.xyzw = (float4)((c255.xyzw >= r10.xxxx));
	ps = clamp(rcp(r0.w), -FLT_MAX, FLT_MAX);
	r0.y = ps;
	r9.w = (float)((r0.z + -r11.x));
	...
	ps = r11.x - r11.y;
	r9.z = ps;
	r1.x = (float)((dot(r2.xwy, r2.xwy)));
	ps = r11.y - r11.z;
	r9.y = ps;
	r1.yzw = (float3)((max(r0.xyz, c252.xxx)));
	ps = r11.z - r11.w;
	r9.x = ps;
	r8.z = (float)((dot(r9.xyzw, c252.zzzz)));
	ps = r3.x - r3.y;
	r0.y = ps;
	r9.xyzw = (float4)((r9.zyxw * c252.zzzz));
	ps = dofProj.x + r0.y;
	r0.x = ps;
	r3.xyz = (float3)((r9.www * r4.xyz + r10.yzw));
	r3.xyz = (float3)((r9.xxx * r5.xyz + r3.xyz));
	r9.xw = (float2)((r8.yz + c252.zw));
	ps = clamp(rcp(r0.x), -FLT_MAX, FLT_MAX);
	r0.y = ps;
	r1.yzw = (float3)((r1.yzw * ToneMapParams.zzz));
	ps = clamp(rcp(r9.w), -FLT_MAX, FLT_MAX);
	r0.x = ps;
	r3.xyz = (float3)((r9.yyy * r6.xyz + r3.xyz));
	r3.xyz = (float3)((r9.zzz * r7.xyz + r3.xyz));
	r3.xyzw = (float4)((r3.xyzw * r0.xxxy));
	ps = max(NoiseParams.z, NoiseParams.z);
	r0.y = (float)((-r3.w + dofDist.w));
	ps = c253.w * ps;
	r0.x = ps;
	r0.z = (float)((r3.w + -dofDist.w));
	ps = clamp(rcp(dofDist.x), -FLT_MAX, FLT_MAX);
	r9.y = ps;
	r0.yz = (float2)((-dofDist.yy * c252.ww + r0.yz));
	r0.yz = (float2)((max(r0.yz, c252.xx)));
	ps = clamp(rcp(dofDist.z), -FLT_MAX, FLT_MAX);
	r9.z = ps;
	r0.xyz = (float3)((r0.yzx * r9.yzx));
	ps = dofBlur.z - dofBlur.y;
	r8.y = ps;
	r8.xy = (float2)((r0.xy * r8.xy + dofBlur.yy));
	r0.xy = (float2)((min(r8.xy, dofBlur.xz)));
	ps = max(-r5.w, -r5.w);
	r8.y = (float)((max(r0.x, r0.y)));
	ps = r4.w + ps;
	r8.x = ps;
	r0.xy = (float2)((r8.yx * r8.yx));
	r1.x = (float)((r1.x + r0.y));
	ps = r2.z * r2.z;
	r2.x = ps;
	r1.x = (float)((r2.x >= r1.x));
	ps = c252.y - r0.x;
	r2.x = ps;
	r1.x = (float)((r2.x * r1.x + r0.x));
	r2.xyzw = (float4)((r1.yzwx * c254.xxxx));
	ps = c252.y - r1.x;
	r1.x = ps;
	r0.xyz = (float3)((r2.www * r7.xyz + r0.zzz));
